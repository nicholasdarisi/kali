...
class CANMaster:
    def __init__(
        ...
        self.ids_stop_on_alert = True  # se True: termina la simulazione quando scatta
    ...
    def report_bit_error(self, sender_slave_id: int, offender: str = "unknown"):
        ...
        if pre_state is NodeState.ERROR_PASSIVE:
            ...
            print(f"[MASTER] {sender.name}: TEC={sender.tec} (+8 per errore PASSIVE) State={sender.state.name}")
                        # --- IDS observe + alert (PASSIVE) ---
            # ---------- IDS: usa il bitstream del SENDER (non il field del master) ----------
            ...
            bs = getattr(sender, "_bitstream", None)
            fields = getattr(bs, "fields", None) if bs is not None else None

            # Se non ho fields/cursor valido, non posso fare "bit concentration" corretta
            if fields is None or bit_idx_abs < 0 or bit_idx_abs >= len(fields):
                # fallback: registra comunque su indice assoluto (meglio di niente)
                self.ids.observe_collision(arb_id=arb_id, bit_idx=bit_idx_abs, offender_name=sender.name)
            else:
                sender_field = fields[bit_idx_abs]

                # Conta solo se l'errore avviene mentre il SENDER è in DATA
                if sender_field is Field.DATA:
                    # Converti indice assoluto -> posizione dentro i DATA bit (0..)
                    data_pos = 0
                    for j in range(bit_idx_abs):
                        if fields[j] is Field.DATA:
                            data_pos += 1

                    self.ids.observe_collision(arb_id=arb_id, bit_idx=data_pos, offender_name=sender.name)
            ...
            if alert:
                print(
                    ...
                    f"top_off={alert['top_offender']} ({alert['top_offender_share']*100:.1f}%)"
                )
                if getattr(self, "ids_stop_on_alert", False):
                    sys.stdout.flush()
                    sys.stderr.flush()
                    os._exit(0)
            # -------------------------------------------------------------------------------
