...
# When SocketCAN forwarding is enabled, real CAN *error flags* cannot be transmitted as frames.
# As an alternative, we can emit a normal data frame that *represents* an error event.
ERROR_DIAG_ARB_ID = 0x7FE  # reserved-ish for simulator diagnostics (11-bit)
ERROR_DIAG_MAGIC = 0xE0    # first byte marker
...
class CANMaster:
    def __init__(
        ...
        forward_error_as_diag: bool = True,
        ...
        self.forward_error_as_diag = bool(forward_error_as_diag)
        self.hw_bus = None
    ...
    def _handle_idle(self):
        ...
        # If multiple intend to start, they contend.
        ...
        print(f"[MASTER] IDLE→ARBITRATION contenders={','.join(self.slaves[s].name for s in self._contenders)}")
    ...
    def _handle_arbitration_step(self):
        ...
        # Arbitration: contenders sending recessive while bus dominant lose (only in ID/RTR; SOF is always dominant).
    ...
    def _handle_transmit_step(self):
        ...
        # Print each bit (verbose)
        print(f"[MASTER] TX bit: sender={sender.name} field={field.name} SENT={sent_bit.name} BUS={bus_bit.name}")

        # Broadcast
        self._broadcast(bus_bit, sender_name=sender.name, field=field)

        # Let sender perform bit monitoring
        sender.on_bus_bit(bus_bit, field, arbitration_window=self._arbitration_window)

        # Advance sender cursor only if no error was raised.
    ...
    def _handle_eof(self):
        ...
        if self._frame_ok and self._winner is not None:
            sender = self.slaves[self._winner]
            req = sender._pending_req
            if req is not None:
                print(f"[MASTER] EOF: frame OK -> forward DATA id=0x{req.arbitration_id:03X} data={req.data.hex().upper()}")
                sender.apply_success_update()
                # Frame consumed
                sender._pending_req = None
                sender._bitstream = None
                sender._cursor = 0
                if self.forward and self.hw_bus:
                    try:
                        msg = can.Message(arbitration_id=req.arbitration_id, data=req.data, is_extended_id=False)
            ...
            if self._error_sender is not None:
                ...
                if self.forward and self.hw_bus:
                    if not self.forward_error_as_diag:
                        print("[MASTER] SocketCAN: error-diag forwarding disabled")
                    else:
                        try:
                            # SocketCAN does not transmit physical CAN error flags as frames.
                            # We forward a *diagnostic* data frame so you can see it in candump.
                            req = sender._pending_req
                            rid = req.arbitration_id if req is not None else 0
                            etype = 1 if sender.state is NodeState.ERROR_ACTIVE else 2
                            # payload: [MAGIC, etype, node_id, TEC, REC, field, rid_hi, rid_lo]
                            field_code = int(getattr(self._current_field, 'value', 0)) & 0xFF
                            payload = bytes([
                            ERROR_DIAG_MAGIC,
                            etype,
                            sender.node_id & 0xFF,
                            ...
                            msg = can.Message(arbitration_id=ERROR_DIAG_ARB_ID, data=payload, is_extended_id=False)
                            self.hw_bus.send(msg)
                            print(f"[MASTER] SocketCAN: sent ERROR_DIAG id=0x{ERROR_DIAG_ARB_ID:03X} payload={payload.hex().upper()}")
                        except Exception as e:
                            print(f"[MASTER] SocketCAN error-diag send failed: {e}")
...
def main():
    print("CAN master-slave bit-level simulator (safe test mode)")

    # Flip r0 once every 5 frames (testing). Set to 0 to disable.
    fault = FaultInjector(flip_r0_every_n_frames=0)

    master = CANMaster(
        tick_ms=0.2,
        forward_to_socketcan=True,  # enable SocketCAN forwarding
        can_channel="vcan0",
        fault_injector=fault,
    )

    # Two ECUs aligned to request simultaneously (arbitration demo)
    ecu_a = BaseECU("ECU_A", 1, 0x100, master, period=3.0, start_delay=0.2)
    ecu_b = BaseECU("ECU_B", 2, 0x200, master, period=3.0, start_delay=0.2)

    # Another ECU that starts later (demonstrates "bus busy -> queued")
    ecu_c = BaseECU("ECU_C", 3, 0x080, master, period=5.0, start_delay=3.0)
    ...
    try:
        for t in range(25):
            ...
            print(
                f"\n[STATUS @ {t+1:02d}s] "
                f"A:TEC={ecu_a.tec:3d}({ecu_a.state.name}) "
                f"B:TEC={ecu_b.tec:3d}({ecu_b.state.name}) "
                f"C:TEC={ecu_c.tec:3d}({ecu_c.state.name})"
            )
