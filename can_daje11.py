...
class CANMaster:
    ...
    def _handle_error_flag_step(self):
        ...
        if flag_type == "ACTIVE":
            # Invia UNA SOLA VOLTA su SocketCAN un frame "000000" (6 byte a 0)
            if getattr(self, "_sockcan", None) is not None and not getattr(self, "_err_active_sent", False):
                for sid in active_senders:
                    ecu = self.slaves[sid]

                    # Provo a ricavare l'arbitration_id dell'ECU in modo robusto
                    arb_id = None
                    for attr in ("req", "_pending_req", "current_req", "_current_req", "_last_req"):
                        req = getattr(ecu, attr, None)
                        if req is not None and hasattr(req, "arbitration_id"):
                            arb_id = req.arbitration_id
                            break
                    if arb_id is None:
                        arb_id = getattr(ecu, "arbitration_id", None)
                    if arb_id is None:
                        arb_id = 0x7FF  # fallback

                    # "6 bit 0" non esiste su SocketCAN -> mando un frame con DLC=6 e 6 byte = 0x00
                    sockcan_send_data(self._sockcan, int(arb_id), b"\x00" * 6)

                self._err_active_sent = True

            # Sul tuo simulatore continuiamo a forzare BUS=0 (dominant)
