...
class CANBusMaster:
    ...
    def _handle_collision(self, bitstreams, active, start_bit_idx):
        """Gestisce collisione tra ECU con stesso ID (bit monitoring -> err flag).

        Modello:
        - Offender ERROR_ACTIVE  -> Error Flag ACTIVE (dominant) -> frame abortito.
        - Offender ERROR_PASSIVE -> Error Flag PASSIVE (recessive) -> non disturba il bus:
          l'altro contendente (attaccante) completa il frame.
        """
        bit_idx = start_bit_idx
        error_detected = False
        offender_ecu = None
        offender_idx = None
        offender_bus_bit = None
        ...
        while len(active) > 0 and not error_detected:
            ...
            # Bit monitoring: se una ECU mette recessivo ma legge dominante -> ERROR (solo dopo RTR)
            for i, ecu, bit, _ in offered:
                if bit is BitValue.RECESSIVE and bus_bit is BitValue.DOMINANT:
                    if field not in {Field.SOF, Field.ID, Field.RTR}:
                        print(f"[MASTER] BIT ERROR rilevato da {ecu.name} (bit monitoring) -> error flag")
                        error_detected = True
                        offender_ecu = ecu
                        offender_idx = i
                        offender_bus_bit = bus_bit
                        break

            bit_idx += 1

        if not (error_detected and offender_ecu is not None):
            return None, None, None, None
        ...
        # REC +1 per i listener non coinvolti
        ...
        # Se l'offender è ERROR_ACTIVE, l'error flag dominant rende invalido il frame -> abort
        if offender_ecu.state is not NodeState.ERROR_PASSIVE:
            return None, None, None, None

        # Se l'offender è ERROR_PASSIVE, il suo passive flag è recessivo:
        # l'altro contendente continua e completa.
        continuing = None
        for i in active:
            if i == offender_idx:
                continue
            ...
            try:
                if offender_bus_bit is not None and bs.bits[bit_idx - 1] is offender_bus_bit:
                    continuing = (ecu, data, arb_id, r0)
                    break
            except Exception:
                continue
        ...
        if continuing is None:
            for i in active:
                if i != offender_idx:
                    ecu, _, arb_id, data, r0 = bitstreams[i]
                    continuing = (ecu, data, arb_id, r0)
                    break
        ...
        cont_ecu, cont_data, cont_id, cont_r0 = continuing
        print(f"[MASTER] ERROR_PASSIVE: {offender_ecu.name} non disturba -> {cont_ecu.name} completa il frame (ID=0x{cont_id:03X})")
        self._transmit_frame(cont_ecu, cont_data, cont_id, cont_r0, [])
        return cont_ecu, cont_data, cont_id, cont_r0
...
# Alias per compatibilità (se nel tuo file locale usi questo nome)
class WeepingAttackingAttacker(WeepingAttacker):
    pass
# --- Main ---
def main():
    print("="*100)
