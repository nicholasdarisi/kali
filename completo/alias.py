...
class CANBusMaster:
    ...
    def _handle_collision(self, bitstreams, active, start_bit_idx):
        ...
        # Listener REC (manteniamo la stessa logica del simulatore: un errore è stato osservato sul bus)
        for ecu in self.ecus.values():
            if ecu not in involved and not ecu.is_bus_off():
                ecu.rec += 1
                print(f"[MASTER] {ecu.name}: REC={ecu.rec} (+1 per errore ricevuto)")
...
class WeepingAttacker:
    def __init__(self, master: CANBusMaster, sniff_duration_1=150, sniff_duration_2=30):
        ...
        self.max_reset_per_round = 16  # massimo reset frames per round (best effort, _reduce_tec può interrompersi)
    ...
    def _create_attack_message(self) -> bytes:
        """Crea il payload d'attacco (DATA 8 byte) allineato alle slide:
        - costruisce una stima MAP del payload della vittima bit-a-bit
        - sceglie un bit dove P(victim=0) è alta (quindi victim dominants spesso)
        - flippa QUEL bit a 1 (recessive) -> recessive injection
        - la scelta del bit è RANDOMIZZATA (pesata), non sempre lo stesso
        """

        import math
        import random

        # Default se non ho info
        probs0 = [0.5] * 64
        probs1 = [0.5] * 64
        weights = [1.0] * 64
        ...
        # 1) Predizione MAP del payload vittima
        predicted_bits = [0 if probs0[i] >= probs1[i] else 1 for i in range(64)]

        # 2) Candidati: bit dove prevediamo 0 (dominant), con score alto = P0 * weight,
        #    MA penalizziamo i bit già usati spesso (anti-sticking).
        candidates = []
        for i in range(64):
            if predicted_bits[i] == 0:
                base = probs0[i] * weights[i]
                # penalità: più è stato usato, meno è probabile ri-selezionarlo
                penalty = (1.0 + self._bit_attack_counts[i]) ** 0.70
                score = base / penalty
                candidates.append((i, score))

        if not candidates:
            # fallback: payload tutto 0
            return bytes([0] * 8)

        # Top-K candidati più “promettenti”
        candidates.sort(key=lambda x: x[1], reverse=True)
        topk = candidates[: max(1, int(self.attack_topk_bits))]

        # 3) Scelta randomizzata:
        #    - con eps -> uniforme sui topK
        #    - altrimenti -> softmax(score/temp)
        ...
        print(
            f"[ATTACKER] 🎯 Randomized target DATA bit {chosen_pos} (Byte {byte_idx}, Bit {bit_idx}): "
            f"P0≈{conf0:.1f}% | flip 0->1"
        )
    ...
    def _attack_loop(self):
        ...
        while self.running and not self.is_bus_off():
            ...
            if self.tec <= self.tec_soft_limit:
                ...
                # da soft_limit a hard_limit: scala linearmente 1.0 -> 0.25
                span = max(1.0, float(self.tec_hard_limit - self.tec_soft_limit))
                p_attack = 1.0 - 0.75 * min(1.0, (self.tec - self.tec_soft_limit) / span)
                p_attack = max(0.25, min(1.0, p_attack))

            do_attack = (random.random() < p_attack)

            if not do_attack:
                print(f"[ATTACKER] ⏭️ Skip attack this cycle (TEC={self.tec}, p_attack={p_attack:.2f}) -> ripulisco TEC")
                if self.tec > 0:
                    self._reduce_tec(num_messages=min(self.max_reset_per_round, 6))
                time.sleep(0.01)
                continue
            ...
            attack_count += 1
            print(f"\n{'='*60}")
            print(f"[ATTACKER] 🎯 ATTACK #{attack_count} on victim {hex(self.victim_id)} | TEC={self.tec} | p_attack={p_attack:.2f}")
            print(f"{'='*60}")
            ...
            if self.tec > 0:
                if self.tec <= self.tec_soft_limit:
                    n_reset = 10   # compensa +8 e tende a mantenere attaccante sotto controllo
                else:
                    n_reset = self.max_reset_per_round  # più aggressivo se TEC sta salendo
