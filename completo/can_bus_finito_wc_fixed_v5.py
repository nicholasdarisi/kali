...
class CANBusMaster:
    ...
    def _handle_collision(self, bitstreams, active, start_bit_idx):
            ...
            for _, ecu, bit, _ in offered:
                if bit is BitValue.RECESSIVE and bus_bit is BitValue.DOMINANT:
                    if field not in {Field.SOF, Field.ID, Field.RTR}:
                        print(f"[MASTER] BIT ERROR (ACTIVE) rilevato da {ecu.name} (offender=bit_monitoring) -> error flag dominante, frame abortito")
                        error_detected = True
                        offender_ecu = ecu
                        break
        ...
        if error_detected and offender_ecu:
            self._send_error_flag_active(offender_ecu, active, bitstreams)

            # Incrementa TEC di tutte le ECU coinvolte nella collisione (come nel tuo modello: +8 a entrambi)
            for i in active:
                ecu = bitstreams[i][0]
                ecu.tec += 8
                ecu._update_state()
    ...
    def _send_error_flag_active(self, offender_ecu, active, bitstreams):
...
class WeepingAttacker:
    def __init__(self, master: CANBusMaster, sniff_duration_1=150, sniff_duration_2=30):
        ...
        self.reset_frames_per_round = 9             # ~neutralizza +8 TEC dell'attacco e tiene attacker "sotto" la vittima
    ...
    def _reduce_tec(self, num_messages: int = None):
        """Riduce il TEC inviando frame con un ID basso (priorità alta) e *non usato* da altre ECU."""
        if self.reset_id is None:
            ...
            # Evita di scegliere un ID "troppo" basso (starvation delle ECU):
            # lo vogliamo con priorità alta, ma vicino alla vittima.
            base = (self.victim_id - random.randint(1, 5)) if self.victim_id is not None else 0x100
            candidate = max(int(base), 0x010)  # evita 0x000..0x00F

            # Se collide con ID già osservati, scendi di poco finché trovi un buco.
            tries = 0
            while candidate in observed and candidate > 0x010 and tries < 32:
                candidate -= 1
                tries += 1
            ...
            self.reset_id = int(candidate)

        reset_id = self.reset_id
        if num_messages is None:
            num_messages = self.reset_frames_per_round

        # NON inviare reset se la vittima sta per trasmettere (altrimenti le rubi la finestra).
        # IMPORTANTISSIMO: non sparare i reset in burst troppo veloce, altrimenti li metti in coda
        # e il master li drena dopo, rischiando di interferire col prossimo periodo della vittima.
        if self.victim_ecu is not None:
            period = float(getattr(self.victim_ecu, "tx_period", 3.0))
            gap = max(0.03, min(0.20, (0.7 * period) / max(1, int(num_messages))))
        else:
            gap = 0.05
    ...
    def _attack_loop(self):
        ...
        while self.running and not self.is_bus_off():
            ...
            if self.tec > 0:
                ...
                self._reduce_tec(num_messages=self.reset_frames_per_round)
