...
def main():
    ...
    # (If you enable flip_r0_every_n_frames, a node can "go in error da sola" because the bus
    #  shows a dominant bit on r0 while it transmits recessive, which triggers bit monitoring.)
    ...
    # ------------------------------------------------------------
    # TEST 1: True arbitration (different IDs, simultaneous request)
    # In CAN, the numerically smaller arbitration ID wins (more dominant 0s earlier).
    # ------------------------------------------------------------
    ...
    # Queue "simultaneously" (START_SLOT will collect both before starting arbitration).
    ecu_a.request_once(b"\x11\x11\x11\x11\x11\x11\x11\x11")
    ecu_b.request_once(b"\x22\x22\x22\x22\x22\x22\x22\x22")
    time.sleep(2)

    # ------------------------------------------------------------
    # TEST 2: Same ID (arbitration tie). If data differs, a bit error happens after RTR.
    # If data is identical, both nodes transmit in lockstep and the bus just sees one frame.
    # ------------------------------------------------------------
    ...
    # Same ID, different DATA -> collision/bit monitoring error (you'll see ERROR_FLAG).
